...
import de.teutonstudio.zentralbank.schnittstelle.anzeigeText
...
fun SpielZustand.zuProzugAnzeigeZustand(): ProzugAnzeigeZustand? {
    ...
    val verwaltung = plan.verwaltungsVerpflichtungen.map { verpflichtung ->
        val versorgung = buildList {
            if (verpflichtung.bedarf.isNotEmpty()) {
                add("Spielerbedarf: ${verpflichtung.bedarf.mengenText()}")
            }
            if (verpflichtung.eigenversorgung.isNotEmpty()) {
                add("Eigenproduktion deckt ${verpflichtung.eigenversorgung.mengenText()}")
            }
            if (isEmpty()) add("kein Versorgungsbedarf")
        }.joinToString(" · ")
        VerwaltungsAnzeige(
            ecke = verpflichtung.id.ecke,
            text = "${verpflichtung.typ.name.lowercase()} ${verpflichtung.id.ecke}: $versorgung",
