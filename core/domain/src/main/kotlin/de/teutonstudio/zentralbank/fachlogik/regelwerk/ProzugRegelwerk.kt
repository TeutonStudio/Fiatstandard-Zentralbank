...
internal object ProzugRegelwerk {
    fun beginnen(
        ...
        val ertraegeNachVerwaltungsstandort = karte?.let {
            KartenAuswertung.abbauErtragNachVerwaltungsstandort(
                it,
                zug.spieler,
                zustand.konflikte,
            )
        ...
        val verwaltung = karte?.let {
            ...
            versorgungsverpflichtung(
                ...
                bruttoErtrag = ertraegeNachVerwaltungsstandort[standort.ecke].orEmpty(),
            )
        }
        val automatischVersorgt = verwaltung
            .filter { verpflichtung -> verpflichtung.bedarf.isEmpty() }
            .mapTo(mutableSetOf(), VerwaltungsVerpflichtung::id)
        val automatischeErtraege = rohstoffeSummieren(
            verwaltung
                .filter { verpflichtung -> verpflichtung.id in automatischVersorgt }
                .map(VerwaltungsVerpflichtung::auszahlbarerErtrag),
        )
        ...
        val mitSnapshot = zustand.copy(
            zugStatus = zug.copy(
                prozug = zug.prozug.copy(
                    begonnen = true,
                    abbauErtraege = automatischeErtraege,
                    verwaltungsVerpflichtungen = verwaltung,
                    verbindlichkeiten = verbindlichkeiten,
                    versorgteStandorte = automatischVersorgt,
                ),
            ),
        )
        return if (automatischeErtraege.isEmpty()) {
            mitSnapshot
        } else {
            RohstoffRegelwerk.rohstoffeBuchen(
                zustand = mitSnapshot,
                spieler = zug.spieler,
                mengen = automatischeErtraege,
                faktor = 1,
            )
        }
    }
    ...
    fun verarbeiten(
        ...
        val verwaltungsstandort = VerwaltungsVerpflichtungId(
            zugId = zug.zugId,
            ecke = standort.verwaltungsstandort,
        )
        require(verwaltungsstandort in zug.prozug.versorgteStandorte) {
            "Der zugehörige Verwaltungsstandort ist noch nicht versorgt."
        }
    ...
    fun verwaltungsstandortVersorgen(
        ...
        val nachVerbrauch = if (verpflichtung.bedarf.isEmpty()) {
            zustand
        } else {
            RohstoffRegelwerk.rohstoffeBuchen(
                zustand = zustand,
        ...
        val nachErtrag = if (verpflichtung.auszahlbarerErtrag.isEmpty()) {
            nachVerbrauch
        } else {
            RohstoffRegelwerk.rohstoffeBuchen(
                zustand = nachVerbrauch,
                spieler = zug.spieler,
                mengen = verpflichtung.auszahlbarerErtrag,
                faktor = 1,
            )
        }
        return nachErtrag.copy(
            zugStatus = zug.copy(
                prozug = zug.prozug.copy(
                    ...
                    abbauErtraege = rohstoffeSummieren(
                        listOf(zug.prozug.abbauErtraege, verpflichtung.auszahlbarerErtrag),
                    ),
                ),
            ),
        )
    ...
    private fun versorgungsverpflichtung(
        id: VerwaltungsVerpflichtungId,
        typ: EckGebaeudeTyp,
        bedarf: Map<Rohstoff, Int>,
        ...
        return VerwaltungsVerpflichtung(
            id = id,
            typ = typ,
            bedarf = restbedarf,
            eigenversorgung = eigenversorgung,
            auszahlbarerErtrag = auszahlbarerErtrag,
        )
    }

    private fun rohstoffeSummieren(
        posten: Iterable<Map<Rohstoff, Int>>,
    ): Map<Rohstoff, Int> = posten
        .flatMap { mengen -> mengen.entries }
