...
object KartenAuswertung {
    ...
    fun abbauErtrag(
        ...
        spieler: SpielerId,
        konflikte: Set<Konflikt> = emptySet(),
    ): Map<Rohstoff, Int> = abbauErtragsQuellen(karte, spieler, konflikte)
        .flatMap { quelle -> quelle.ertrag.entries }
        .groupBy(Map.Entry<Rohstoff, Int>::key, Map.Entry<Rohstoff, Int>::value)
        .mapValues { (_, mengen) -> mengen.sum() }

    /**
     * Ordnet jeden automatischen Abbauertrag genau dem Verwaltungsstandort zu, dessen
     * Anschlussstärke verwendet wird. Ein einfacher Schienenanschluss wird vom Hauptbahnhof
     * verwaltet. Diese Zuordnung erlaubt, Ertrag erst nach der Standortversorgung freizugeben.
     */
    fun abbauErtragNachVerwaltungsstandort(
        karte: Spielkarte,
        spieler: SpielerId,
        konflikte: Set<Konflikt> = emptySet(),
    ): Map<KartenEcke, Map<Rohstoff, Int>> = abbauErtragsQuellen(karte, spieler, konflikte)
        .groupBy(AbbauErtragsQuelle::verwaltungsstandort)
        .mapValues { (_, quellen) ->
            quellen
                .flatMap { quelle -> quelle.ertrag.entries }
                .groupBy(Map.Entry<Rohstoff, Int>::key, Map.Entry<Rohstoff, Int>::value)
    ...
    private fun abbauErtragsQuellen(
        karte: Spielkarte,
        spieler: SpielerId,
        konflikte: Set<Konflikt>,
    ): List<AbbauErtragsQuelle> = karte.belegung.felder.mapNotNull { belegung ->
        val weg = transportWeg(karte, belegung.position, spieler, konflikte)
        ...
        when (val anlage = belegung.anlage) {
            ...
            .mapValues { (_, ertrag) -> ertrag * weg.anschlussStaerke }
            .takeIf(Map<Rohstoff, Int>::isNotEmpty)
            ?.let { ertrag ->
                AbbauErtragsQuelle(
                    verwaltungsstandort = verantwortlicherVerwaltungsstandort(karte, weg),
                    ertrag = ertrag,
                )
            }
    }
    ...
    private fun verantwortlicherVerwaltungsstandort(
        karte: Spielkarte,
        weg: TransportWeg,
    ): KartenEcke {
        val startGebaeude = karte.belegung.eckenNachPosition[weg.start]
        return if (
            startGebaeude?.besitzer == weg.spieler &&
            startGebaeude.zustand == BauwerkZustand.INTAKT
        ) {
            startGebaeude.position
        } else {
            weg.hauptbahnhof
        }
    }
    ...
    private data class AbbauErtragsQuelle(
        val verwaltungsstandort: KartenEcke,
        val ertrag: Map<Rohstoff, Int>,
    )
